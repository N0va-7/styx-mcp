...
- **异步 `start_cmd`**（非交互 `sh -c`，返回 `task_id`）
- **异步 `start_scan`** — 从指定 agent 做内网侦察（探活 → 端口 → 轻量指纹 + 漏洞 **refs**，不做利用）
- **异步 `pull_file`** / `upload_file`（任务 + 本地路径；非交互 shell）
...
| `start_cmd` | 非交互一次性命令 | Agent `sh -c`（异步 `task_id`） |
| `start_scan` | 内网端口扫描 + 轻量指纹 | **Agent** 网络栈出站（异步 `task_id`） |
...
| 从 **agent 侧** 直接拿结构化开放端口/指纹（不必先起本机 SOCKS） | `start_scan` |
...
结果常用字段：`stats`（`hosts_alive`、`discover_ms`、`method`、`fallback` 等）、`open[]`、`summary.interesting[]`、可选 `warnings[]`。  
`refs` 是 **提示**（CVE/通告链接），不是已确认漏洞。
...
- `start_scan` 仅侦察（connect/SYN + 轻量指纹 + 通告链接）。不要把 refs 当已确认漏洞。改 SCAN\* 后须 **同 commit** 重编 controller 与 agent。
